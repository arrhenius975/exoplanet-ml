...
def _metric_variable(name, shape, dtype):
  """Creates a Variable for metrics that tracks across batches.
  
  Uses tf.compat.v1.Variable with explicit collections for compatibility
  with Estimator evaluation loops.
  """
...
def _build_metrics(labels, predictions, weights, batch_losses, output_dim=1):
  ...
  if binary_classification:
    ...
    predicted_labels = tf.argcompat.v1.assign_add(num_examples, tf.reduce_sum(weights))
    ...
    metrics["accuracy/num_correct"] = (num_correct.read_value(),
                                       update_num_correct)
    accuracy = tf.math.dividetric_variable("num_correct", [], tf.float32)
    is_correct = tf.equal(labels, predicted_labels)
    weighted_is_correct = weights * tf.cast(is_correct, tf.float32)
    ...
    def _count_condition(name, labels_value, predicted_value):
      ...
      update_op = tf.compat.v1.assign_add(count, tf.reduce_sum(weighted_is_equal))
