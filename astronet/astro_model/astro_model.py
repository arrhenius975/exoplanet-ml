...
class AstroModel(object):
  ...
  def build(self):
    ...
    # Create global step variable for tracking training progress
    try:
      self.global_step = tf.train.get_or_create_global_step()
    except:
      # Fallback for TF2: create explicit variable
      self.global_step = tf.Variable(0, trainable=False, name="global_step")

    if self.mode == tf.estimator.ModeKeys.TRAIN:
      # Use a conditional: in training mode, is_training defaults to True
      # but can be overridden via a placeholder for in-process validation
