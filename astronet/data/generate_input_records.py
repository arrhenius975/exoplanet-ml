...
def _process_file_shard(tce_table, file_name):
  ...
  logging.info("%s: Processing %d items in shard %s", process_name,
  ...
  with tf.io.TFRecordWriter(file_name) as writer:
    ...
    for _, tce in tce_table.iterrows():
      ...
      if not num_processed % 10:
        logging.info("%s: Processed %d/%d items in shard %s", process_name,
                     num_processed, shard_size, shard_name)

  logging.info("%s: Wrote %d items in shard %s", process_name, shard_size,
               shard_name)
...
def main(argv):
  ...
  logging.info("Launching %d subprocesses for %d total file shards",
